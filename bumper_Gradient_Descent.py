...
def func(args):
    ...
    testBumper = Bumper((0.5 * 0.0254 + mag_unc) / width_to_r, l1, d1, L, phi,
                        (0.5 * 0.0254 + mag_unc) / width_to_r, l2, d2, 500, start=st, long_off=0.0,
                        focus_off=-0.03, real_mag=(1 / 2 * 0.0254, 3 / 4 * 0.0254, 1 / 2 * 0.0254))
    return testBumper.cost(size_cost=True, alignment_cost=False)


def main():
    xi_meters = np.array([0.19668369, 0.37725694, -0.03887307, 0.09760237, 0.09681795, 0.22661218, -0.0119319732])
    xi = xi_meters / norms
